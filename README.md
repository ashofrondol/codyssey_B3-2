...
> 저장소 구성은 `main.py`(473줄) + `README.md`(119줄) 2개 파일이 전부이며, README 주장은 모두 소스에서 재확인했다. 추가로 실제 REPL 세션과 합성 그래프(diamond)로 실행 검증했다(아래 🧪 절).
...
4. **(경미, 명세 외) 저장소 위생** — 컴파일 산출물 `__pycache__/main.cpython-314.pyc` 가 커밋에 포함돼 있고(`git ls-files`) `.gitignore` 가 없다.
   → `.gitignore` 에 `__pycache__/`, `*.pyc` 추가 후 `git rm -r --cached __pycache__` 권장.
